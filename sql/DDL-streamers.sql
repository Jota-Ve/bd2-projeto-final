...
CREATE TABLE public.inscricao (
    ...
    FOREIGN KEY (nro_plataforma_fk, id_canal_fk, nivel) REFERENCES public.nivel_canal(nro_plataforma_fk, id_canal_fk, nivel) ON UPDATE CASCADE ON DELETE CASCADE
);
...
SELECT 
    c.nro_plataforma_fk AS nro_plataforma,
...
FROM 
    public.canal c
LEFT JOIN 
    public.inscricao i ON c.nro_plataforma_fk = i.nro_plataforma_fk AND c.id_canal = i.id_canal_fk
LEFT JOIN 
    public.patrocinio p ON c.nro_plataforma_fk = p.nro_plataforma_fk AND c.id_canal = p.id_canal_fk
GROUP BY 
    c.nro_plataforma_fk, c.nome, c.qtd_visualizacoes;

-- 3. Video Engagement
CREATE OR REPLACE VIEW public.vw_video_engagement AS
SELECT 
    v.nro_plataforma_fk AS nro_plataforma,
...
FROM public.video v
LEFT JOIN public.comentario cm
  ON v.nro_plataforma_fk = cm.nro_plataforma_fk AND v.id_video = cm.id_video_fk
LEFT JOIN public.doacao d
  ON cm.id_comentario = d.id_comentario_fk
GROUP BY v.nro_plataforma_fk, v.id_video, v.titulo, v.visu_total;
...
FROM 
    public.plataforma p
LEFT JOIN 
    public.canal c ON p.nro = c.nro_plataforma_fk
LEFT JOIN 
    public.video v ON c.nro_plataforma_fk = v.nro_plataforma_fk AND c.id_canal = v.id_canal_fk
GROUP BY 
...
CREATE INDEX idx_comentario_video ON public.comentario(nro_plataforma_fk, id_video_fk);
